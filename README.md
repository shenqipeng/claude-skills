...
- **Expanded MCU database** from 284+ to 651 models, 5 → 8 manufacturers
  - Added **MindMotion (灵动微)** — 36 models (MM32F0010~F5277, MM32L, MM32SPIN series)
  - Added **Artery (雅特力)** — 23 models (AT32F403/F413/F415/F421/F425/F402/F405, AT32L021/L010)
  - Added **APT (爱普特)** — 18 models (APT32F101~F173, RISC-V + C-Sky cores)
  - Expanded GigaDevice: 85 → 283 models
  - Expanded Fudan Micro: 25 → 109 models
  - Expanded WCH: 40 → 84 models
  - Expanded XHSC: 18 → 27 models
  - SiLabs unchanged: 71 models
- **Upgraded database schema** (Schema A → Schema B):
  - Unified field names: `packages[]` array, `adc_resolution` numeric, `peripherals[]` list
  - Added fields: `pins`, `gpio`, `adc_resolution`, `can`/`usb`/`ethernet` as booleans
  - Removed `statistics` section for simpler DB structure
- **Rewrote mcu-selection SKILL.md** — 4-step linear workflow → interactive 7-phase Q&A
  - Phase 0: Pre-fill from PRD/brainstorming context
  - Phase 1: Guided questioning (one at a time, 7 questions)
  - Phase 2: Early termination rules (≤5 candidates → stop)
  - Phase 3: Ranked results with comparison table
  - Added edge cases: no matches, too many matches, specific MCU lookup
  - Added 3 usage examples (from scratch, partial context, PRD context)
- **Cleanup**: removed deprecated `mcu_database_complete.json` and `mcu_database_filtered.json`
- **Added `research/` directory** with per-manufacturer source data (10 files)
- Updated README.md and SKILLS_GUIDE.md (284+ → 650+, 5 → 8 manufacturers)

---
name: test-cases
description: "Generate structured test cases from PRD/requirements. Chain: upstream←product-requirements(acceptance criteria); downstream→test-driven-development(RED input),verification-before-completion(all must pass),writing-plans(ref acceptance criteria). Bridges requirements→testing. Skip if: no PRD,quick hotfix"
license: MIT
---
...
**Position in superpowers workflow:** Bridges requirements to testing — the "test chain" that connects PRD to TDD to verification.

**Upstream (feeds into this skill):**
- `product-requirements` — Primary input. After PRD is generated with acceptance criteria, invoke test-cases to generate structured test cases from those criteria.

**Downstream (receives test cases):**
- `superpowers:test-driven-development` — Generated test cases serve as the "RED" input for TDD. Each test case becomes a failing test that TDD makes pass.
- `superpowers:verification-before-completion` — Before claiming work is complete, verify all generated test cases pass.
- `superpowers:writing-plans` — Reference generated test cases when writing implementation plans, so each plan step has clear acceptance criteria.

**Decision rule:** Invoke test-cases whenever a PRD exists or acceptance criteria are defined. The output bridges the gap between "what to build" (PRD) and "how to verify it works" (TDD + verification). Without test-cases, TDD has no structured input and verification has no checklist.

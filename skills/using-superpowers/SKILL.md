...
|-----------|----------|----------------|
| New project, unclear requirements | `product-requirements` | Skip if ≤3 files AND no new hardware |
...
- **Combine** when: bug fix with traceability requirement → `sparv` with `systematic-debugging` + `freeze` embedded

### Key Cross-Links

- `product-requirements` → `test-cases` → `test-driven-development` → `verification-before-completion` (requirements-to-verification chain)
- `systematic-debugging` → `freeze` (lock scope first) → `careful` (if hw ops) → `TDD` (failing test) → `verification`
- `writing-skills` ↔ `skill-creator` (skill development lifecycle)

...
**If `test-cases` skill has generated structured test cases from a PRD**, use those test cases as the first failing tests. Each test case from `test-cases` maps directly to a RED cycle — write the test, verify it fails, then proceed to GREEN. This bridges requirements to TDD: PRD → test-cases → failing tests → implementation.

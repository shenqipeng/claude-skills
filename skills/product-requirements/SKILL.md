...
description: "Interactive Product Owner. 100-point quality scoring, generates PRD at ≥90. Chain: earliest skill in workflow; downstream→brainstorming(design from PRD),test-cases(generate tests from acceptance criteria),mcu-selection(if embedded). Skip if: ≤3 files,no new hardware,mechanical change,user has clear requirements"
---
...
**Position in superpowers workflow:** This skill is the UPSTREAM entry point BEFORE brainstorming.

**Upstream:** None — this is the earliest skill in the workflow.

**Downstream (must come after this skill):**
- `brainstorming` — When PRD is complete, invoke brainstorming to explore design. The PRD serves as the formal requirements input for brainstorming.
- `test-cases` — After PRD is finalized, invoke test-cases to generate structured test cases from the PRD's acceptance criteria. These test cases bridge to TDD during implementation.
- `mcu-selection` — If the PRD specifies embedded/hardware requirements, invoke mcu-selection to select the target MCU. The MCU choice feeds into brainstorming's design phase.

**Decision rule:** For new projects or features without formal requirements, invoke this skill FIRST. The PRD output becomes the input for `brainstorming` → `writing-plans` → execution.

**Skip condition:** Skip `product-requirements` and go directly to `brainstorming` when:
- Task scope is small (≤3 files affected)
- No new hardware/MCU involved
- Change is mechanical (rename, config update, simple API call)
- User already has clear, concise requirements
- Existing PRD covers the scope
